      ...
      <header className="sticky top-0 z-30 border-b border-[#dfe3ea] bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1440px] flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm font-medium">
            <button className="rounded-md bg-[#176b87] px-4 py-2.5 text-white hover:bg-[#145b73]" onClick={openCreate} type="button">新增客户</button>
            <button className="rounded-md border border-[#d7dce4] bg-white px-3 py-2 text-[#334155]" onClick={() => refresh()} type="button">刷新</button>
            <span className="rounded-md border border-[#d7dce4] bg-white px-3 py-2 text-[#334155]">{session.fullName || session.email}</span>
            <button className="rounded-md bg-[#0d5b4d] px-3 py-2 text-white" onClick={signOut} type="button">退出</button>
          </div>
        </div>
      </header>

      <section className="mx-auto grid max-w-[1440px] gap-5 px-5 py-6">
        <div className="grid gap-3 md:grid-cols-4">
          {stats.map(([label, value]) => <div className="rounded-lg border border-[#dfe3ea] bg-white p-4 shadow-sm" key={label}><p className="text-sm text-[#667085]">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>)}
        </div>
          ...
          <section className="overflow-hidden rounded-lg border border-[#dfe3ea] bg-white shadow-sm">
            <div className="border-b border-[#e5e7eb] bg-[#fbfcfd] p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div><h2 className="text-lg font-semibold">客户管理</h2><p className="mt-1 text-sm text-[#667085]">搜索、筛选、编辑客户，并查看合作与账户资料。</p></div>
                <button className="rounded-md bg-[#176b87] px-4 py-2.5 text-sm font-semibold text-white" onClick={openCreate} type="button">新增客户</button>
              </div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1.6fr)_220px]">
                <label className="grid gap-1.5 text-sm font-medium">搜索<input className="field" onChange={(event) => setQuery(event.target.value)} placeholder="公司、联系人、电话、邮箱、广告账户" value={query} /></label>
                <label className="grid gap-1.5 text-sm font-medium">状态<select className="field" onChange={(event) => setStatusFilter(event.target.value as CustomerStatus | "全部")} value={statusFilter}><option>全部</option>{statuses.map((status) => <option key={status}>{status}</option>)}</select></label>
              </div>
            </div>
            ...
            <div className="max-h-[calc(100vh-310px)] overflow-auto">
              <table className="w-full min-w-[1000px] border-collapse text-left text-sm">
                <thead className="sticky top-0 z-10 bg-[#f8fafc] text-xs uppercase text-[#64748b]"><tr>{["公司", "联系人", "电话", "地区", "行业", "广告账户", "状态", "负责人", "操作"].map((heading) => <th className="px-4 py-3 font-semibold" key={heading}>{heading}</th>)}</tr></thead>
                <tbody>
                  {filteredCustomers.map((customer) => <tr className={`border-t border-[#edf0f4] transition-colors hover:bg-[#f8fbfc] ${selectedCustomer?.id === customer.id ? "bg-[#eef8fb]" : ""}`} key={customer.id}>
                    ...
                    <td className="px-4 py-3"><select className="rounded-md border border-[#d7dce4] bg-white px-2 py-1" disabled={saving} onChange={(event) => handleStatus(customer, event.target.value as CustomerStatus)} value={customer.status}>{statuses.map((status) => <option key={status}>{status}</option>)}</select></td>
                    <td className="px-4 py-3">{customer.ownerName || "-"}</td>
                    <td className="px-4 py-3"><button className="table-action" onClick={() => openEdit(customer)} type="button">编辑</button></td>
                  </tr>)}
                </tbody>
              </table>
